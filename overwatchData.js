...
function roleRankString(roleData) {
  ...
  const division =
    roleData.division.substring(0, 1).toUpperCase() +
    roleData.division.substring(1);
...
function getTotalSkillRating(competitive) {
  ...
  return (
    getSkillRating(competitive.tank) ??
    0 + getSkillRating(competitive.damage) ??
    0 + getSkillRating(competitive.support) ??
    0
  );
}
